...
self.addEventListener('install', e => {
  e.waitUntil(
    ...
    // addAll() rejeita TUDO se um único arquivo faltar — e aí o SW nunca instala
    // e o app perde o offline inteiro, sem erro visível no celular. Cacheia um a um.
    caches.open(CACHE)
      .then(c => Promise.allSettled(STATIC.map(u => c.add(new Request(u, {cache:'reload'})))))
      .then(rs => { const f=rs.filter(r=>r.status==='rejected').length; if(f)console.warn('SW: '+f+' arquivo(s) do cache estático falharam'); })
